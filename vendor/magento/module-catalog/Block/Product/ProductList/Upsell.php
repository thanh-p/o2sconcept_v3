...
class Upsell extends AbstractProduct implements IdentityInterface
{
    ...
    public function getRandomItems($item_num)
    {
        $current_product = $this->getProduct();
        $category = $this->categoryFactory->create()->load($current_product->getCategoryIds()[0]);
        ...
        foreach ($collection as $product) {
            ...
            if ($current_product->getId() == $loaded_product->getId()) {
                continue;
            }

            array_push($outputs, $loaded_product);
            $counter = $counter + 1;
        }
